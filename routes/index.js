...
router.get('/my-account', ensureAuthenticated, (req, res) => {
  Product.find({ user: new Object(req.user) }).then((products) => { res.render('my-account', { user: req.user, products }) })
...
router.get('/shop', ensureAuthenticated, (req, res) => {
  Product.find({ user: { $ne: new Object(req.user) } }).then((products) => { res.render('shop', { user: req.user, products }) })
})
